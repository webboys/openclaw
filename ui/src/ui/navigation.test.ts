...
describe("iconForTab", () => {
  ...
  it("returns stable icons for known tabs", () => {
    ...
    expect(iconForTab("runtime")).toBe("radio");
...
describe("inferBasePathFromPathname", () => {
  ...
  it("returns empty string for direct tab path", () => {
    ...
    expect(inferBasePathFromPathname("/runtime")).toBe("");
  });
...
describe("beginner mode navigation", () => {
  it("marks beginner tabs correctly", () => {
    expect(isBeginnerNavTab("chat")).toBe(true);
    expect(isBeginnerNavTab("overview")).toBe(true);
    expect(isBeginnerNavTab("channels")).toBe(true);
    expect(isBeginnerNavTab("debug")).toBe(false);
  });

  it("filters tab groups in beginner mode", () => {
    const groups = visibleTabGroups({ beginnerMode: true, activeTab: "chat" });
    const tabs = groups.flatMap((group) => group.tabs);
    expect(tabs).toEqual(["chat", "overview", "channels"]);
  });

  it("keeps the active tab visible in beginner mode", () => {
    const groups = visibleTabGroups({ beginnerMode: true, activeTab: "config" });
    const tabs = groups.flatMap((group) => group.tabs);
